            ...
            steps {
                script {
                    if (env.BRANCH_NAME == 'master') {
                        ...
                        slack "App published\n${gitCommit}"
                        sh "(git tag -d published-${BUILD_NUMBER} || echo 'No local tag to delete'); git tag -a published-${BUILD_NUMBER} -m 'Version published-${BUILD_NUMBER}'; (git push --delete origin published-${BUILD_NUMBER} || echo 'No remote tag'); git push --tags; git commit -am 'chore: Upload latest release ${BUILD_NUMBER}'; git pull --rebase; git push"
                    }
                }
            }
        }
